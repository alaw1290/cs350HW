...
public class Scheduler {
	...
	public Scheduler(double lamb, double lr, double CPU, double Net, double Disk, double simulationLength, int f){
	...
	public void insertDeathEvent(double serviceStart, int type){
		...
		//CPU A type 21
		if(type == 21){
			Event death = emitter.nextCPU1Death(serviceStart, flag);
			Schedule.add(death);
		}
		
		//CPU B type 22
		if(type == 22){
			Event death = emitter.nextCPU2Death(serviceStart, flag);
			Schedule.add(death);
		}
		
		//Net type 3
		if(type == 3){
			Event death = emitter.nextNetDeath(serviceStart, flag);
			Schedule.add(death);
		}
		
		//Disk type 4
		if(type == 4){
			Event death = emitter.nextDiskDeath(serviceStart, flag);
			Schedule.add(death);
